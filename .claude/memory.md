...
## [2026-02-07] Phase 2.5 Enhanced Evaluation (Clock/Tempo/Card Quality)
- **Type**: Feature
- **Description**: Added three strategic dimensions to `getScoreForGameStateImpl()` in `GameStateEvaluator.java`: (1) **Clock calculation** — tracks evasive damage (flying/horsemanship/unblockable) per side during battlefield iteration, calculates turns-to-kill, awards ±15 pts per turn of clock advantage (capped ±80). (2) **Tempo score** — compares untapped mana producers between players, ±3 pts per mana source advantage. (3) **Card quality weighting** — hand cards scored by castability: castable spells worth 6 pts, non-castable 3 pts, lands 3 pts (replacing flat 5-per-card).
...
## [2026-02-07] Phase 3.1 MCTS Implementation (EXPERIMENTAL)
- **Type**: Feature
- **Description**: Implemented Monte Carlo Tree Search as alternative to minimax/alpha-beta. New files: `MCTSAction.java` (action identifiers), `MCTSNode.java` (UCB1 tree nodes), `MCTSController.java` (core MCTS loop with select/expand/rollout/backpropagate). Heuristic rollout plays both players using lands-first + CMC ordering. Sigmoid score normalization (SCALE=150). PASS tiebreaker to prevent stalling. New `MCTS.ai` profile, selectable via `-P1 MCTS`. Added `MCTS_ENABLED`, `MCTS_ITERATIONS`, `MCTS_EXPLORATION_CONSTANT`, `MCTS_ROLLOUT_DEPTH` to `AiProps.java`. Added `getDoubleProperty()` to `AiProfileUtil.java`. Modified `AiController.chooseSpellAbilityToPlay()` to dispatch to MCTS when enabled.
- **Why**: MCTS handles large branching factors and hidden info better than minimax in theory. However, **benchmarks show it dramatically underperforms**: MCTS Burn vs Default Faeries = **4% win rate** (Simulation gets 70%); MCTS vs Simulation mirror = **10% win rate**. Root causes: game copy cost ~30-80ms limits iterations to 30-200/decision (need 1000+), and the static evaluation undervalues delayed effects (face damage) at depth 1. Code retained as experimental; **Simulation profile remains recommended**.

## [2026-02-07] Phase 4.1-4.6 CLI Polish (ALL PHASES COMPLETE)
- **Type**: Feature
- **Description**: Implemented all 6 CLI polish items:
  1. **Wilson score confidence intervals** (`WilsonInterval.java`): 95% CI shown in text output (e.g., `65.0% [43.3%, 81.9%]`) and JSON (`winRateCiLower`/`winRateCiUpper` fields on `PlayerSummary`).
  2. **CSV export** (`--csv` flag): Per-game results table + player summary with CI, pipe-friendly for downstream tools.
  3. **Matchup matrix**: Round-robin tournaments print pairwise win rate matrix after standings via `outputMatchupMatrix()`.
  4. **Progress bar** (`ProgressBar.java`): Character-based `[###>    ] 50% 10/20 ETA: 0:33` on stderr, replaces old line-based "Progress: X/Y" output.
